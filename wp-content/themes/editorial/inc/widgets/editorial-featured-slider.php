...
class Editorial_Featured_slider extends WP_Widget {
    ...
    ?>
        ...
        <div class="featured-post-wrapper">
            <?php
                $feaured_args = editorial_query_args( $editorial_featured_category_id, 3 );
                $feaured_query = new WP_Query( $feaured_args );
                $total_post_count = $feaured_query->post_count;
                if( $feaured_query->have_posts() ) {
                    $post_count = 1;
                    while ( $feaured_query->have_posts() ) {
                        $feaured_query->the_post();
                        $post_id = get_the_ID();
